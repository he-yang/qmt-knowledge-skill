...
> ⚠️ **敏感 IP 警示**：导出包虽经加密，但内含**策略逻辑、交易信号、调仓规则等核心知识产权**，泄露可能造成策略被复制、反向利用或抢跑等损失。
> - 分享/传输导出包前，请**确认接收方可信**，并优先使用加密渠道（如企业内网、端到端加密通讯）传递。
> - 加密密码应**独立于策略文件单独送达**接收方，避免密码与策略包一同泄露。
> - 公开渠道（如 GitHub、网盘、即时通讯群组）**切勿上传未脱敏的实盘策略包**；如需演示，请导出不含真实账号与生产参数的脱敏版本。
